...
VOID CCodeTransfer::Hook()
{
	...
	if (ReadBYTE(HookºÏ≤‚1) != 0xEB)
	{
		Action_In_HookVirtualProtect([] {MyTools::CCharacter::WriteBYTE(HookºÏ≤‚1, 0xEB); });
	}
}
...
{
	DWORD dwOldHookVirtualProtectValue1 = ReadDWORD(ª÷∏¥VirtualProtect);
	DWORD dwOldHookVirtualProtectValue2 = ReadDWORD(ª÷∏¥VirtualProtect + 0x4);

	DWORD dwProtect = 0;
	::VirtualProtect(reinterpret_cast<LPVOID>(ª÷∏¥VirtualProtect), 0x8, PAGE_EXECUTE_READWRITE, &dwProtect);
	*reinterpret_cast<DWORD*>(ª÷∏¥VirtualProtect + 0x0) = 0x4DB8;
	*reinterpret_cast<BYTE*>(ª÷∏¥VirtualProtect + 0x4) = 0x0000;

	Ptr();

	*reinterpret_cast<DWORD*>(ª÷∏¥VirtualProtect + 0x0) = dwOldHookVirtualProtectValue1;
	*reinterpret_cast<DWORD*>(ª÷∏¥VirtualProtect + 0x4) = dwOldHookVirtualProtectValue2;
	//::VirtualProtect(reinterpret_cast<LPVOID>(ª÷∏¥VirtualProtect), 0x8, dwProtect, &dwProtect);
}
